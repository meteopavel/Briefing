...
if [[ "$BACKUP_OK" -eq 1 ]]; then
    ...
    log "🔒 Создаём зашифрованный архив (.env, .local_secure/, CLAUDE.md, PRODUCT.md, DESIGN.md, docs/)..."
    (
        cd "${PROJECT_ROOT}"
        7z a -p"${ARCHIVE_PASSWORD}" -mhe=on "${ARCHIVE_PATH}" \
            ".env" "CLAUDE.md" "PRODUCT.md" "DESIGN.md" ".local_secure/" "docs/" > /dev/null 2>&1 || true
    )
